...
public void keyPressed(KeyEvent e) {
  ...
  if ((info.isUp() || info.isDown()) && info.getShip().active && !info.isThrustersPlaying()) {
    if (info.isSound() && !info.isPaused())
      info.getThrustersSound().loop();
    info.setThrustersPlaying(true);;
  }

  // Spacebar: fire a photon and start its counter.

  if (e.getKeyChar() == ' ' && info.getShip().active) {
    if (info.isSound() & !info.isPaused())
      info.getFireSound().play();
    info.setPhotonTime( System.currentTimeMillis());
    info.setPhotonIndex(info.getPhotonIndex()+1);
